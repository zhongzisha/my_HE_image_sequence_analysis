...
def _load_checkpoint_for_ema(model_ema, checkpoint):
    ...
    """
    mem_file = io.BytesIO()
    torch.save(checkpoint, mem_file) # save loaded object to memory
    mem_file.seek(0) # reset file pointer
    model_ema._load_checkpoint(mem_file) # load from memory
...
def init_distributed_mode(args): # set parameters for different processes from environmental variables
    if args.dist_on_itp:
