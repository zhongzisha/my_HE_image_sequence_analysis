...
def get_args():
    ...
    parser.add_argument('--img_path', type=str, help='input video path', default='/mnt/gridftp/zhongz2/Xenium_Prime_Mouse_Brain_Coronal_FF_outs/version8_with_video/rot+000/448/10000/rot0_sub/x9979.902911617994_y17288.90174854475.mp4')
    parser.add_argument('--save_path', type=str, help='save video path', default='/mnt/gridftp/zhongz2/Xenium_Prime_Mouse_Brain_Coronal_FF_outs/version8_with_video/rot+000/448/10000/rot0_sub/x9979.902911617994_y17288.90174854475_vis')
    parser.add_argument('--model_path', type=str, help='checkpoint path of model', default='/scratch/cluster_scratch/zhongz2/VideoMAEv2/OUTPUT/pretrain/checkpoint-199.pth')
...
def main(args):
    ...
    fileprefix = os.path.basename(args.img_path).replace('.mp4', '')
    tcgaprefix = args.img_path.split('/')[-2]
    existed_prefixes = [os.path.basename(f) for f in glob.glob(args.save_path + '/*.jpg')] 
    ...
    for k,v in checkpoint['model'].items():
        checkpoint_model[k.replace('_orig_mod.', '')] = v
    ...
    tmp = np.arange(0, 64-args.num_frames)
    tmp = np.arange(16)
    tmp = np.arange(0, 48, 3)
    tmp = np.array([0, 4, 8, 12, 16, 20, 24, 28, 32, 36, 40, 44, 48, 52, 56, 60])
    ...
    with torch.no_grad():
        ...
        all_ims = []
        for id, (orig_im, mask_im, rec_im) in enumerate(zip(orig_imgs, mask_imgs, rec_imgs)):
            im = np.concatenate([np.array(orig_im), np.array(mask_im), np.array(rec_im)], axis=1)
            all_ims.append(im)
            if id == 0:
                im = Image.fromarray(im)
                im.save(f"{args.save_path}/{tcgaprefix}_{fileprefix}_mask{args.mask_ratio}_patch0.jpg")

        if f'{tcgaprefix}_{fileprefix}_mask{args.mask_ratio}.jpg' not in existed_prefixes:
            all_ims = np.concatenate(all_ims, axis=0)
            all_ims = Image.fromarray(all_ims)
            all_ims.save(f"{args.save_path}/{tcgaprefix}_{fileprefix}_mask{args.mask_ratio}.jpg")
